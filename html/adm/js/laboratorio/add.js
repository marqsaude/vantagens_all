...
function validaCadastroLaboratorio(obj){
    var html = "";
    if(obj.nm_laboratorio.value==""){
        html += "Nome do Laboratório vazio!\n";
    }
    ...
    if(html != "") {
        ...
        var array = {"nm_laboratorio": obj.nm_laboratorio.value, "nu_valor":obj.nu_valor.value, "nu_valor_real":obj.nu_valor_real.value, "prestadores":prestadores };
        ajaxPost(funcaoCadastroLaboratorio, array, "/laboratorio/ajax-add/", "/admin");
    }
    return false;
}

var funcaoCadastroLaboratorio = function(json){
    if(json!=null){
        alert("Cadastrado com Sucesso!");
        window.location.href = getUrlController() + "/admin/laboratorio/add";
    }
};

function submitCadastroLaboratorio(){
    jQuery("#addLaboratorio").submit();
}
...
function addPrestador(){
    ...
    if(typeof te != "undefined") {
        $(".mais").each(function( i ) {
            ...
            jQuery(".mais").remove();
            var prestador = {"id": idPrestador, "nome": nmPrestador.trim()};
