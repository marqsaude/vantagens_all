...
function validaCadastroConsulta(obj){
    var html = "";
    ...
    if(obj.nu_valor.value==0.00 || obj.nu_valor.value=="" || obj.nu_valor.value==0 || obj.nu_valor.value=="0.00"){
        html += "É necessário inserir algum valor na consulta!\n";
    }
    ...
    if(html != "") {
        ...
        var array = {"nm_consulta": obj.nm_consulta.value, "nu_valor":obj.nu_valor.value, "nu_valor_real":obj.nu_valor_real.value, "prestadores":prestadores };
        ajaxPost(funcaoCadastroConsulta, array, "/consulta/ajax-add/", "/admin");
    }
    return false;
}

var funcaoCadastroConsulta = function(json){
    if(json!=null){
        alert("Cadastrado com Sucesso!");
        window.location.href = getUrlController() + "/admin/consulta/add";
    }
};

function submitCadastroConsulta(){
    jQuery("#addConsulta").submit();
}
